...
### Changed
- 生产目标的应用计划强确认简化为输入固定关键字 `APPLY`（容忍大小写与首尾空格），不再要求复制完整长确认文本；计划 ID 与目标环境仅作展示与审计。
- 应用计划确认面板新增执行条件提示：按钮禁用时逐条显示原因（未确认、无选中项、存在阻断项、任务执行中、已执行成功），满足条件时显示"已满足执行条件"。

### Fixed
- 修复配置编辑器光标与所见文字错位的问题：高亮层 `code` 补上 `font: inherit`，消除两层等宽字体宽度差异造成的横向偏占位（预览层与编辑层字宽不一致，越往行尾偏得越多）；textarea 增加 `overscroll-behavior: contain`，切断滚动到底后外层容器被链式滚走导致的光标错行。
- 修复批量替换场景下应用计划在执行前被误判 stale 的问题：freshness 指纹基准改用生成计划时的原始来源值，不受替换后内容影响。
- 修复应用计划确认面板被 dry-run/执行进度块顶起、变更工作台被压塌的布局问题：主列改为纵向滚动，工作台保留保底高度。
- 修复配置浏览分组下拉在纵向筛选栏中被撑到异常高度的问题。
- 移除配置浏览筛选栏中多出的"分组"文字标签（回归）。

### Engineering
- 新增《生产确认流程优化说明》文档；补充应用计划确认输入、执行条件与 stale 判定的回归测试。

## [1.7.0] - 2026-07-23

### Added
- 配置浏览支持按分组筛选，并优化左侧工具条布局与 SVG 图标显示。
- 配置对比页记住上一次左右来源，并在批量对比中标记仅左侧/仅右侧存在的配置。
- 应用计划支持 diff 预览、dry-run 检查和按配置项选择执行。
- Windows 便携版将 WebView 数据保存到 exe 同级目录，并支持从旧 AppData 数据自动迁移。

### Fixed
- 修复 SSH 隧道 localhost/IPv6、过期端口缓存和并发创建导致的连接失败。
- 修复 Nacos 中文响应在 GBK/GB18030 场景下显示乱码的问题。
- 修复对比页和筛选下拉弹层被父容器遮挡的问题。

### Engineering
- 增加便携数据迁移、SSH 隧道复用、配置浏览分组、对比偏好和应用计划 dry-run 的回归测试。
