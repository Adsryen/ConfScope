...
**当前基线**: `v1.0.0`
**当前开发线**: `v1.1.0`
**下一项立即做**: `v1.3.0 / P0` 的云端到本地快照导出与最终快照 schema 收口。
...
- [ ] 全局 SSH 隧道配置档案。
  - [ ] 新增 `SSHProfile` 本地持久化模型：id、名称、host、port、username、authType、password/privateKey/passphrase、localPort 策略。
  - [ ] 连接模型新增 `sshProfileId`，连接表单通过引用复用 SSH 配置；保留旧 `sshConfig` 兼容历史连接。
  - [ ] 连接管理页在 `连接方式 = 通过 SSH 隧道` 时显示“选择 SSH 隧道配置”下拉，支持已有配置、复制为新配置、临时内联配置。
  - [ ] SettingsView 增加 SSH 隧道配置档案管理入口，支持新增、编辑、删除、测试连接、查看被哪些连接引用。
  - [ ] 删除或修改 SSH 配置档案前提示影响范围；被引用时默认禁止直接删除，允许确认后解绑或迁移引用。
  - [ ] 凭据字段继续按敏感信息处理，日志、导出和测试错误中不得泄露密码、私钥、passphrase。
  - [ ] 自动化测试覆盖配置档案持久化、连接引用、旧 `sshConfig` 迁移兼容、删除影响提示。
- [ ] SSH 隧道状态完善。
  - 显示已连接、本地端口、失败原因。
  - 支持手动重连和断开。
  - 连接删除或修改时自动关闭旧隧道。
...
- [ ] 主布局骨架。
  ...
  - [ ] 侧边栏图标视觉继续统一到最终主题。
- [ ] SSH 运行时隧道实例复用。
  - [ ] 以 `sshProfileId + targetHost + targetPort` 作为运行时隧道复用 key；目标地址不同的连接不共享同一个本地端口。
  - [ ] 后端 SSH manager 维护引用计数，多个连接使用同一隧道时只建立一次，最后一个引用释放后再关闭。
  - [ ] 隧道失效、重连、手动断开时同步影响引用该实例的连接状态展示。
  - [ ] localPort 固定配置存在冲突时明确提示；默认仍建议自动分配端口。
  - [ ] 覆盖单元测试：相同 profile/目标复用、不同目标隔离、断开引用计数、失败状态传播。
- [ ] 工程质量。
  - 配置 ESLint。
  - 配置 Prettier。
