...
**当前基线**: `v1.0.0`
**当前开发线**: `v1.1.0`
**下一项立即做**: `v1.1.0 / P1` 的全局消息中心与智能对比错误聚合，避免并发请求产生多个阻塞弹窗。
...
- [ ] 正在优化全局错误与通知体验：将错误从默认弹窗改为消息中心记录 + toast 摘要 + 主动查看详情。
...
- [ ] 全局代理进入 SettingsView。
  ...
  - UI 标识当前连接是否走代理。
- [ ] 全局消息中心与错误聚合。
  - [ ] 将当前 `errorCenter` 升级为消息中心：支持 `error`、`warning`、`info`、`success` 等级，保留完整详情、来源、时间、重复次数和可选操作。
  - [ ] 左侧栏底部新增消息入口：显示未读数量，支持展开消息面板。
  - [ ] 消息面板支持查看详情、复制完整内容、标记已读、清空消息。
  - [ ] Toast 只展示短摘要，不再用多个阻塞弹窗承载并发错误。
  - [ ] 错误详情弹窗改为用户主动从消息中心或 toast 打开；发布、删除等关键阻塞操作可保留主动弹详情。
  - [ ] 智能对比错误聚合：300-500ms 内同类错误合并；同一 `mergeKey` 重复错误计数，不连续弹窗。
  - [ ] 批量对比失败生成汇总消息：展示成功数、失败数、失败 dataId 列表，并可复制完整详情。
