...
**任何新功能、优化或 bug 修复，必须同步编写自动化测试，测试绿灯后才能视为完成。**

| 变更类型 | 测试要求 |
|----------|----------|
| 新组件 | 必须有组件测试，覆盖核心交互和错误路径 |
| 新 API 函数 | 必须有单元测试，覆盖成功和失败路径 |
| 新 lib 函数 | 必须有纯函数测试，覆盖边界情况 |
| UI 优化 | 必须确保现有测试通过，必要时更新测试 |
| Bug 修复 | 必须添加回归测试，覆盖修复的场景 |

**测试绿灯门禁：**
- 前端：`pnpm exec tsc --noEmit` + `pnpm test`
- 后端：`go test ./... -count=1`
...
- [x] v1.2.0 全部 P0/P1 任务已完成：审计矩阵 UI、i18n、导出 CSV/JSON、过滤排序、矩阵行跳转 DiffView、AuditView ↔ DiffView 打通。175/175 测试全绿。
...
- [ ] 全局消息中心与错误聚合。
  ...
  - [x] 消息面板支持查看详情、复制完整内容、标记已读、清空消息。
  - [x] Toast 只展示短摘要，不再用多个阻塞弹窗承载并发错误。
  - [x] 错误详情弹窗改为用户主动从消息中心或 toast 打开；发布、删除等关键阻塞操作可保留主动弹详情。
  - [x] 智能对比错误聚合：300-500ms 内同类错误合并；同一 `mergeKey` 重复错误计数，不连续弹窗。
  - [x] 批量对比失败生成汇总消息：展示成功数、失败数、失败 dataId 列表，并可复制完整详情。
  - [ ] 为后续任务/备份/同步进度复用消息模型，避免再新增分散状态。
- [x] 连接管理升级为 provider 感知表单。
  - [x] 独立连接管理页面第一版。
  - [x] 项目 / 环境 / 来源层级管理第一版。
  - [x] 项目、环境重命名与新增来源入口。
  - [x] 环境固定下拉：开发、测试、预发、生产、灰度、本地；兼容历史环境值。
  - [x] 来源入口与连接方式分离：来源描述云上公网、云上内网、公司内网、办公网络等目标位置；SSH 仅作为连接辅助方式。
  - [x] 来源类型第一版：Nacos 在线连接、本地快照目录。
  - [x] 本地快照目录录入：支持手输路径、Wails 目录选择、保存前校验。
  - [x] 表单帮助提示第一版：字段帮助图标、非透明浮层、边缘防遮挡。
  - [x] provider 类型选择。
  - [x] 根据 provider 切换字段。
  - [x] 基础信息、认证、网络、安全、高级分组收口。
  - [x] 连接列表显示 provider、环境标签、安全等级、SSH/代理状态。
- [ ] 全局 SSH 隧道配置档案。
  - [x] 新增 `SSHProfile` 本地持久化模型：id、名称、host、port、username、authType、password/privateKey/passphrase、localPort 策略。
  - [x] 连接模型新增 `sshProfileId`，连接表单通过引用复用 SSH 配置；保留旧 `sshConfig` 兼容历史连接。
  - [x] 连接管理页在 `连接方式 = 通过 SSH 隧道` 时显示“选择 SSH 隧道配置”下拉，支持已有配置、复制为新配置、临时内联配置。
  - [x] 新增独立 SSH 隧道管理页面，承载 SSH 配置档案新增、编辑、删除、测试连接和引用关系查看。
  - [x] SettingsView 只保留语言和应用级偏好，SSH 档案管理迁出设置页。
  - [x] 删除 SSH 配置档案前提示影响范围；被引用时默认禁止直接删除。
  - [x] SSH 配置档案支持独立测试连接。
  - [x] 修改 SSH 配置档案前提示影响范围，保存前需要二次确认。
  - [ ] 必要时支持复制为新档案后迁移引用。
  - [ ] 凭据字段继续按敏感信息处理，日志、导出和测试错误中不得泄露密码、私钥、passphrase。
  - [x] 自动化测试覆盖配置档案持久化、连接引用、旧 `sshConfig` 兼容。
  - [x] 自动化测试覆盖删除影响提示。
- [ ] SSH 能力边界收口。
  - 当前阶段只保留配置中心访问所需的 SSH 档案、引用关系、测试连接和影响范围提示。
  - 不把 ConfScope 做成 SSH 隧道运维控制台；运行态隧道状态、手动断开/重连、实例复用仅在配置访问性能或稳定性确有瓶颈时再评估。

#### P2

- [ ] 主布局骨架。
  - [x] 从顶部双模式切换升级为左侧主导航第一版。
  - [x] 左侧主导航承载：浏览、对比、连接管理、审计、备份、任务、设置、关于。
  - [x] 侧边栏支持图标、收起与展开。
  - [x] 移除与系统标题栏重复的应用 topbar。
...
- [x] 矩阵行跳转。
  ...
  - 跳转时自动选择基准环境 vs 第一个不匹配环境。
- [x] AuditView ↔ DiffView 打通。
  - DiffView 保持深度对比定位。
  - AuditView 作为发现层，DiffView 作为详情层。
  - 矩阵行点击 → 跳转 DiffView 深度对比。
...
- [ ] 本地快照目录来源管理。
  - [x] 连接管理页支持添加本地快照目录来源。
  - [x] Windows/Wails 桌面端支持系统目录选择。
  - [x] Windows/Linux/macOS 支持手动输入目录路径。
  - [x] 保存前强制校验目录，避免任意目录直接参与对比。
  - [x] 当前 MVP 校验规则：目录根部包含 `confscope.snapshot.json` / `manifest.json` / `metadata.json`，或 `configs/` / `namespaces/` 标准目录，并至少找到 1 个可对比配置文件。
  - [x] 选择目录后自动校验，并展示校验路径和配置文件数量。
  - [ ] 与最终导出快照 schema 对齐后，收紧校验规则。
- [ ] 云端到本地快照。
  - 支持从 Nacos 拉取一次快照保存到本地。
  - 快照保存原始内容和元信息。
- [ ] local provider。
  - [x] 本地快照作为 `local` provider 参与智能对比。
  - [x] 支持云端 vs 本地备份的只读对比入口。
  - [x] 支持本地备份 A vs 本地备份 B 的只读对比入口。
  - [x] 智能对比页面可选择同项目同环境下的云端来源与本地来源。
  - [x] `public` 目录映射为 Nacos 默认命名空间。
  - [x] local provider 禁止发布、删除、历史写入类操作。
  - [ ] 接入配置浏览页，把 local provider 也作为浏览来源。
  - [ ] 与最终导出快照 schema 对齐后，支持读取 metadata 中的更新时间、版本、来源等字段。
- [x] BackupView。
  - [x] 快照列表。
  - [x] 快照详情。
  - [x] 云端/本地来源标识。
  - [x] 快照与当前云端的差异入口。
  - [x] 配置中心快照 WebDAV 同步：独立 `.cssnapshot` 加密包、上传、远端列表、下载导入为本地快照。
  - [x] Docker WebDAV smoke 覆盖上传、列表、导入、导入后参与 DiffView。
...
- [x] AuditView UI：i18n 国际化、忽略规则、名称归一化、状态摘要、矩阵表格、详情面板。
- [x] 审计结果导出：CSV / JSON 双格式，脱敏开关。
- [x] 审计过滤与排序：状态过滤、隐藏已忽略、按列排序。
- [x] AuditView ↔ DiffView 打通：矩阵行跳转深度对比。
- [x] 侧边栏优化：分组显示、分隔线、分组标签、命名优化、SSH 隧道合并。
- [x] 操作历史：本地记录存储、配置中心历史拉取、筛选、详情面板。
